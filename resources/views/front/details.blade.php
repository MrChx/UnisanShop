    ...
    <body>
        <div class="relative flex flex-col w-full max-w-[640px] min-h-screen gap-5 mx-auto bg-[#F5F5F0]">
            <div id="top-bar" class="flex justify-between items-center px-4 mt-[60px]">
                ...
                <p class="font-bold text-lg leading-[27px]">Detail Jajanan</p>
            ...
            <section id="gallery" class="flex flex-col gap-[10px]">
                ...
                <div class="swiper w-full overflow-hidden">
                    <div class="swiper-wrapper">
                        ...
                        @foreach ($food->photos as $itemPhoto)
            ...
            <section id="info" class="flex flex-col gap-[14px] px-4">
                <div class="flex items-center justify-between">
                    <h1 id="title" class="font-bold text-2xl leading-9">{{ $food->name }}</h1>
                    <div class="flex flex-col items-end shrink-0">
                        <div class="flex items-center gap-1">
                            <span class="font-semibold text-xl leading-[30px]">{{ $food->stock }} Stok</span>
                        </div>
                        <p class="text-sm leading-[21px] text-[#878785]">{{ $food->category->name }}</p>
                    </div>
                </div>
                <p id="desc" class="leading-[30px]">{{ $food->about }}</p>
            </section>
            <div id="brand" class="flex items-center gap-4 px-4">
                <div class="w-[70px] h-[70px] rounded-[20px] bg-white overflow-hidden">
                    <img src="{{ Storage::url($food->seller->logo) }}" class="w-full h-full object-contain" alt="brand logo">
                </div>
                <div class="flex flex-col">
                    <h2 class="text-sm leading-[21px]">Penjual</h2>
                    <div class="flex items-center gap-1">
                        <h3 class="font-bold text-lg leading-[27px]">{{ $food->seller->name }}</h3>
                        <img src="{{asset ('assets/images/icons/arrow-left.svg')}}" class="w-5 h-5" alt="icon">
                    </div>
                </div>
            </div>
            
            <form action="{{ route('front.save_order', $food->slug) }}" method="POST" class="flex flex-col gap-3">
                @csrf
                    ...
                    <div class="fixed bottom-5 w-full max-w-[640px] z-30 px-4">
                        <div class="flex items-center justify-between rounded-full bg-[#2A2A2A] p-[10px] pl-6">
                            <div class="flex flex-col gap-[2px]">
                                <p class="font-bold text-[20px] leading-[30px] text-white">Rp {{ number_format($food->price, 0, ',', '.') }}</p></p>
                                <p class="text-sm leading-[21px] text-[#878785]">Harga 1 kemasan</p>
                            </div>
